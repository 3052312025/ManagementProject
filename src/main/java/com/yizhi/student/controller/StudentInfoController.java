...
public class StudentInfoController {
	...
	public R save(StudentInfoDO studentInfoDO){
		...
		try {
			studentInfoDO.setAddTime(dateFormat.parse(dateFormat.format(date)));
		}catch (Exception e){
	...
	public PageUtils list(@RequestParam Map<String, Object> params){
		...
		List<StudentInfoDO> studentsList = studentInfoService.list(query);
		int total = studentInfoService.count(query);
		PageUtils pageUtils = new PageUtils(studentsList, total,query.getCurrPage(),query.getPageSize());
		return pageUtils;
	...
	public R update(StudentInfoDO studentInfo){
		...
		SimpleDateFormat dateFormat= new SimpleDateFormat("yyyy-MM-dd :hh:mm:ss");
		try {
			studentInfo.setEditTime(dateFormat.parse(dateFormat.format(date)));
		}catch (Exception e){
			System.out.println(e);
		}
